...
esse tipo de situação, não que a empresa não tem solução."""
    ...
    reflexoes = pacote_saida.get("perguntas_e_reflexoes", [])
    reflexoes_texto = ""
    if reflexoes:
        blocos = []
        for r in reflexoes:
            bloco = f"- Pergunta feita ao empresário: \"{r['pergunta']}\"\n  Resposta dele: \"{r['resposta']}\""
            if r.get("analogia"):
                bloco += f"\n  Analogia disponível (use no máximo uma, só se ajudar a explicar, nunca force): {r['analogia']}"
            if r.get("se_resposta_sim"):
                bloco += f"\n  O que uma resposta afirmativa geralmente revela: {r['se_resposta_sim']}"
            if r.get("se_resposta_nao"):
                bloco += f"\n  O que uma resposta negativa geralmente revela: {r['se_resposta_nao']}"
            blocos.append(bloco)
        reflexoes_texto = "\n\n".join(blocos)

    instrucao_reflexao = ""
    if reflexoes_texto:
        instrucao_reflexao = f"""

Perguntas feitas durante a investigação, com a resposta do empresário e o
que cada resposta costuma revelar:
{reflexoes_texto}

Ao escrever a seção "por que essa decisão foi escolhida" (item 2 da ordem
acima), incorpore a pergunta que mais pesou na decisão e a resposta que o
empresário deu, de forma natural - mostre a ele que a decisão nasceu
diretamente do que ele mesmo respondeu, não de um cálculo abstrato. Não
liste as perguntas separadamente nem crie uma seção nova para isso; teça
isso dentro do "por quê". Use a analogia apenas se genuinamente ajudar a
tornar a explicação mais concreta - nunca force uma analogia que não se
encaixe bem."""
    ...
    exemplos = [
        ...
        {"tipo": "decisao_concluida", "constraint_dominante": "Dependência comercial do fundador",
         ...
         "perguntas_e_reflexoes": [
             {"pergunta": "Algum projeto foi fechado sem o fundador?",
              "resposta": "Não, todos os projetos foram fechados por ele.",
              "analogia": "É como uma bicicleta com uma roda só.",
              "se_resposta_sim": "Indica musculatura comercial própria.",
              "se_resposta_nao": "Comum em empresas que cresceram pela rede do fundador."}
         ]},
    ]
    for ex in exemplos:
